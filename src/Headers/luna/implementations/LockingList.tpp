...
template<template<typename listType> class ListType, typename T> void LockingList<ListType, T>::add(T elements[],
																									const size_t
																											listLength)
{
	list.insert(list.end(), elements, elements[listLength - 1]);
}
template<template<typename listType> class ListType, typename T> void LockingList<ListType,
																				  T>::add(std::initializer_list<T>
																								  &&elements)
{
	list.insert(list.end(), elements.begin(), elements.end());
}
template<template<typename listType> class ListType, typename T> void LockingList<ListType, T>::add(LockingList
																											&elements)
{
	list.insert(list.end(), elements.list.begin(), elements.list.end());
}
template<template<typename listType> class ListType, typename T> LockingList<ListType, T> LockingList<ListType, T>::
operator+(const LockingList &rightSide)
{
	add(rightSide);
	return this;
}
template<template<typename listType> class ListType, typename T> LockingList<ListType, T> LockingList<ListType, T>::
operator+(const std::initializer_list<T> &&rightSide)
{
	add(rightSide);
	return this;
}
